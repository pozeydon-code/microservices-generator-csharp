...
public sealed class DomainResult<T>
{
    ...
    public static DomainResult<T> Success(T value) => value is null ? throw new ArgumentNullException(nameof(value)) : new(value, []);
    public static DomainResult<T> Failure(params DomainError[] errors) => errors.Length == 0 ? throw new ArgumentException("At least one error is required.", nameof(errors)) : new(default, errors);
}
