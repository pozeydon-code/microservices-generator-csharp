...
func TestRunGenerateSucceeds(t *testing.T) {
	...
	expectedStdout := "Generated 58 files in " + outputDir + "\n"
...
func TestRunTUISucceedsWithRunnerSeam(t *testing.T) {
	...
	runTUIProgram = func(plan application.GenerationPlan, request application.GenerateRequest, planFunc tui.PlanFunc, generate tui.GenerateFunc, update tui.UpdateSettingsFunc, updateServices tui.UpdateServicesFunc, updateEntities tui.UpdateEntitiesFunc, updateFields tui.UpdateFieldsFunc, updateValueObjects tui.UpdateValueObjectsFunc, targetFrameworkSuggestions []string) error {
		...
		if refreshedPlan.FileCount != 58 || refreshedPlan.OutputDir != outputDir {
		...
		if result.Plan.FileCount != 58 || result.OutputDir != outputDir {
		...
		if !updateResult.Saved || updateResult.PlanError != nil || updateResult.Plan.Config.SolutionName != "CatalogPlatform" || updateResult.Plan.Config.TargetFramework != "net9.0" || updateResult.Plan.FileCount != 58 {
		...
		if !entityResult.Saved || entityResult.PlanError != nil || entityResult.Plan.Config.EntityCount != 1 || entityResult.Plan.FileCount != 58 {
		...
		if !fieldResult.Saved || fieldResult.PlanError != nil || fieldResult.Plan.Config.EntityCount != 1 || fieldResult.Plan.FileCount != 58 {
	...
	if capturedPlan.OutputDir != outputDir || capturedPlan.FileCount != 58 || capturedPlan.OutputAction != "create" || capturedPlan.ForceUsed {
