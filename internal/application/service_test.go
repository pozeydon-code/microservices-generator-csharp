...
func TestServiceUpdateRelationshipSettingsSavesAndSummarizesOneToOneRelationships(t *testing.T) {
	saver := &fakeConfigSaver{}
	service := NewService(Ports{
		ConfigLoader:    &fakeConfigLoader{cfg: validConfigWithRelationshipEntities()},
		ConfigSaver:     saver,
	...
	result, err := service.UpdateRelationshipSettings(GenerateRequest{ConfigPath: "microgen.json", OutputDir: "generated"}, RelationshipSettings{ServiceName: "ProductService", Relationships: []RelationshipSetting{{Name: "ProductCategory", Multiplicity: "one-to-one", PrincipalEntity: "Category", DependentEntity: "Product", ForeignKeyName: "CategoryId", ForeignKeyType: "Guid", Required: &required, DependentNavigation: "Category"}}})

	if err != nil {
		t.Fatalf("expected one-to-one relationship update success, got %v", err)
	}
	if !saver.called || saver.path != "microgen.json" {
		t.Fatalf("expected config save at microgen.json, got called=%v path=%q", saver.called, saver.path)
	}
	relationships := saver.cfg.Services[0].Relationships
	if len(relationships) != 1 || relationships[0].Multiplicity != "one-to-one" || relationships[0].PrincipalNavigation != "" {
		t.Fatalf("expected editable one-to-one relationship to be saved without forcing navigation defaults, got %#v", relationships)
	}
	wantSummary := []RelationshipSummary{{Name: "ProductCategory", Multiplicity: "one-to-one", PrincipalEntity: "Category", DependentEntity: "Product", ForeignKeyName: "CategoryId", ForeignKeyType: "Guid", Required: true, PrincipalNavigation: "Product", DependentNavigation: "Category", Summary: "Category 1-1 Product via CategoryId (required)"}}
	if !reflect.DeepEqual(result.Config.Services[0].Relationships, wantSummary) {
		t.Fatalf("expected one-to-one relationship summary %#v, got %#v", wantSummary, result.Config.Services[0].Relationships)
	}
	if !containsString(result.Plan.Readiness.Hints, "Review 1 relationship navigation before generating.") {
		t.Fatalf("expected relationship readiness hint, got %#v", result.Plan.Readiness.Hints)
	}
...
func TestServiceUpdateRelationshipSettingsRejectsUnsupportedOptionsWithoutSaving(t *testing.T) {
	tests := []struct {
		...
		{name: "many-to-many", multiplicity: "many-to-many"},
		{name: "blank", multiplicity: ""},
	}
...
func TestDefaultServicePlanGenerationUsesRealPortsWithoutWriting(t *testing.T) {
	...
	if plan.FileCount != 69 || len(plan.Files) != 69 {
		t.Fatalf("expected 69 planned files, got count=%d len=%d", plan.FileCount, len(plan.Files))
	}
	if plan.Files[0].Path != "Directory.Build.props" {
