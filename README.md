...
The TUI is organized as a step-based dashboard: `Source`, `Project`, `Services`, `Preview`, and `Generate`. Use `tab` or `]` to move forward and `shift+tab` or `[` to move backward. `Source` confirms the JSON config source and output path, `Project` edits solution metadata and target framework, `Services` can add, rename, or delete services, `Preview` shows impact and planned files, and `Generate` focuses final confirmation/status/result.
...
Use `up`/`down`, `k`/`j`, `pgup`/`pgdown`, `home`, and `end` to inspect planned files on the `Preview` step, `a` to cycle the planned-file action filter, `r` to refresh the plan from disk, `e` to edit the active `Project` or `Services` step, `g` to generate, and `q`, `esc`, or `ctrl+c` to quit. Project edit mode updates solution name, description, and target framework. Services edit mode uses `up`/`down` to select, `a` to add, `r` to rename, `d` to delete when more than one service remains, `enter` to save, and `esc` to cancel. Target framework editing shows installed .NET SDK major-version suggestions first when `dotnet --list-sdks` is available; otherwise it falls back to a known newest-first list. You can also type a major or TFM manually, such as `6`, `7`, `net10.0`, or `net11.0`. Exit is blocked while saving settings or generating files. Entity, field, and value-object editing is not supported yet.
...
- The TUI can edit solution metadata, target framework, and the service list. Entity, field, and value-object editing remains future work.
