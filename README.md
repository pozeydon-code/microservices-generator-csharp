...
The TUI is organized as a step-based dashboard: `Source`, `Project`, `Services`, `Preview`, and `Generate`. Use `tab` or `]` to move forward and `shift+tab` or `[` to move backward. `Source` confirms the JSON config source and output path, `Project` edits solution metadata and target framework, `Services` can add, rename, or delete services and basic entities, `Preview` shows impact and planned files, and `Generate` focuses final confirmation/status/result.
...
- The TUI can edit solution metadata, target framework, services, and basic entity names. Field and value-object editing remains future work.
