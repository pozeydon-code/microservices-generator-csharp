...
The TUI now opens in a route-based workspace with `Overview`, `Project`, `Services`, `Preview`, and `Generate` screens. Wide terminals show a persistent navigation rail; medium and narrow terminals use compact top navigation. Use `up`/`down` or `j`/`k` to select a route, `enter` to open it, `h`/`l` or `left`/`right` to switch screens, `1`-`5` for direct routes, and `?` for help. `esc` backs from the Project editor to Project and from other screens to Overview; `q` and `ctrl+c` quit.

The workspace shell is the first incremental redesign slice. Existing Services, Preview, and Generate behavior remains reachable, while resource editor migration continues incrementally from Services.
