...
`microgen tui` opens a terminal UI over the same planning and generation core. It shows the loaded config summary, output action, force status, and planned files before anything is written. Use `up`/`down`, `k`/`j`, `pgup`/`pgdown`, `home`, and `end` to inspect planned files, `r` to refresh the read-only plan from disk, `g` to generate, and `q`, `esc`, or `ctrl+c` to quit. Exit is blocked while generation is writing files. The TUI does not edit config files yet.
