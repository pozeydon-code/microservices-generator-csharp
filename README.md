...
The TUI is organized as a workspace route shell: `Source`/`Overview`, `Project`, `Services`, `Entities`, `Value Objects`, `Preview`, `Generate`, and `Result`. Use `tab` or `]` to move forward and `shift+tab` or `[` to move backward outside the Services resource context. `Source` confirms the JSON config source and output path, `Project` edits solution metadata and target framework, `Services` is a service workspace, `Entities` is a dedicated entity list/detail workspace, and `Value Objects` is a dedicated value-object list/detail workspace. The dedicated resource routes retain nested Fields and Value Object Rules editors. `Preview` validates readiness, output mode, force safety, impact counts, deleted files, and the planned-file list; `Generate` presents a readiness checklist and waits for an explicit `g` confirmation; successful generation opens `Result` with output counts, deleted-file details, warnings, and the next build command. If a save succeeds but plan refresh fails, readiness is shown as stale and generation stays locked until refresh succeeds.
