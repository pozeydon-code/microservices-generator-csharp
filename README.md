...
The TUI is organized as a workspace route shell: `Source`/`Overview`, `Project`, `Services`, `Entities`, `Value Objects`, `Preview`, and `Generate`. Use `tab` or `]` to move forward and `shift+tab` or `[` to move backward outside the Services resource context. `Source` confirms the JSON config source and output path, `Project` edits solution metadata and target framework, `Services` is a service workspace, `Entities` is a dedicated entity list/detail workspace, and `Value Objects` is a dedicated value-object list/detail workspace. The dedicated resource routes retain nested Fields and Value Object Rules editors, while `Preview` shows impact and planned files and `Generate` focuses final confirmation/status/result. The `Generate` route also shows a compact readiness summary with project/service/entity/field/value-object counts, whether output replacement needs `--force`, and concise next-action hints for starter configs. After successful generation, it shows a compact impact summary with output directory, planned file count, action counts, cleanup count when applicable, and the next build command. If a save succeeds but plan refresh fails, readiness is shown as stale and generation stays locked until refresh succeeds.
...
The TUI now opens in a route-based workspace with `Overview`, `Project`, `Services`, `Entities`, `Value Objects`, `Preview`, and `Generate` screens. Wide terminals show a persistent navigation rail; medium and narrow terminals use compact top navigation with stacked resource content. Use `up`/`down` or `j`/`k` to select a route or resource, `enter` to open it, `h`/`l` to switch screens, `left`/`right` or `tab` to switch the Services context, `1`-`7` for direct routes, and `?` for help. On Services, the contexts are `Services`, `Entities`, and `Value Objects`; `up`/`down` selects the service or resource in the active context, and `enter` opens the corresponding dedicated route/editor. The Entities route shows the selected service, entity list, field names/types, and referenced value objects. The Value Objects route shows the selected service, value-object list, type, compact validation rules, and referencing fields. `e` edits the active resource list, `f` opens Fields from Entities, `o` opens Rules from Value Objects, and `v` opens Value Objects from Services. Inside resource editors, `a` adds, `r` renames, `d` deletes, `enter` saves, and `esc` cancels. `esc` backs Entities and Value Objects to the matching Services context; nested Fields and Rules return to their parent resource route. `q` and `ctrl+c` quit.
