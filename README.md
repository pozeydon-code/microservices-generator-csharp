...
- `--output`: directory where files are published.
...
- Uses no Go runtime dependencies outside the standard library.
...
- Nested/composed Value Objects and the TUI are later slices; this generator remains CLI-first.
...
3. Add TUI as an adapter over the existing spec/generation core.
