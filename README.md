...
The TUI is organized as a step-based dashboard: `Source`, `Project`, `Services`, `Preview`, and `Generate`. Use `tab` or `]` to move forward and `shift+tab` or `[` to move backward outside the Services resource context. `Source` confirms the JSON config source and output path, `Project` edits solution metadata and target framework, `Services` is a resource workspace for services, entities, fields, value objects, and basic value-object rules, `Preview` shows impact and planned files, and `Generate` focuses final confirmation/status/result. The `Generate` step also shows a compact readiness summary with project/service/entity/field/value-object counts, whether output replacement needs `--force`, and concise next-action hints for starter configs. After successful generation, it shows a compact impact summary with output directory, planned file count, action counts, cleanup count when applicable, and the next build command. If a save succeeds but plan refresh fails, readiness is shown as stale and generation stays locked until refresh succeeds.
...
The TUI now opens in a route-based workspace with `Overview`, `Project`, `Services`, `Preview`, and `Generate` screens. Wide terminals show a persistent navigation rail; medium and narrow terminals use compact top navigation. Use `up`/`down` or `j`/`k` to select a route, `enter` to open it, `h`/`l` to switch screens, `left`/`right` or `tab` to switch the Services context, `1`-`5` for direct routes, and `?` for help. On Services, the contexts are `Services`, `Entities`, and `Value Objects`; `up`/`down` selects the service or resource in the active context, and `enter` opens its editor. `e` edits the service list, `f` opens fields from the selected entity editor, and `v` opens value objects. Inside resource editors, `a` adds, `r` renames, `d` deletes, `enter` saves, and `esc` cancels. `esc` backs nested fields editing to the Services workspace and backs root screens to Overview; `q` and `ctrl+c` quit.
